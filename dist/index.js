...
export class AdoClient {
     ...
     */
    async updateWorkItem(id, rev, fields, maxRetries = 1) {
        let currentRev = rev;
                ...
                if (current === undefined || attempt >= maxRetries) {
                    throw new CommandError(`work item ${id} changed since it was read: asserted rev ${rev}` +
                        (current !== undefined
                            ...
                            : " and could not be re-read"), EXIT_CODE.conflict);
                }
        ...
        throw new CommandError(`work item ${id} could not be updated: maxRetries must not be negative (got ${maxRetries})`, EXIT_CODE.usage);
    }
}
/**
